...
export default function ChecklistHistoryPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        console.log('Loading checklists for user:', user?.email);
        ...
        console.log('Loaded checklists:', data.length);
        setChecklists(data);
        setFilteredChecklists(data);
         ...
         for (const branchId of uniqueBranchIds) {
           try {
             console.log('Loading template for branch:', branchId);
             const templateData = await getTemplate(branchId);
             if (templateData) {
               templatesData[branchId] = templateData;
               console.log('Template loaded for branch:', branchId);
             }
           } catch (error) {
         ...
         console.log('Loaded templates for branches:', Object.keys(templatesData));
  ...
  const estimateChecklistCategory = (checklist: ChecklistRecord): 'daily' | 'weekly' | 'monthly' => {
    ...
    return 'daily';
     ...
     // 디버깅용 로그
     console.log(`Checklist ${checklist.id} (${checklist.branchId}):`, {
       category: estimatedCategory,
       totalItems: checklist.items.length,
       completedItems: checklist.items.filter(item => item.checked).length,
       requiredItems: requiredItemIds.length,
       completedRequiredItems,
       completionRate,
       templateLoaded: !!branchTemplate,
