...
export function OrderEditDialog({ isOpen, onOpenChange, order }: OrderEditDialogProps) {
  ...
  const createDeliveryExpense = async (order: Order, actualCost: number) => {
    try {
      ...
      const supplier = formData.recipient.name || '운송업체'; // NOTE: using recipient name as proxy if driver info not available, but wait...
      // In OrderEditDialog, we don't have driver info fields (only recipient). 
      // pickup-delivery/page.tsx has distinct driver info.
      // Here we only have recipient info. 
      // Wait, if it's delivery reservation, `deliveryInfo` has `recipientName` etc.
      // But we don't present Driver Name/Contact in this dialog?
      // Checking the form... It seems this dialog is for ORDER INFO, not DRIVER INFO.
      // Driver info is usually handled in pickup-delivery page.
      // BUT user asked to input actual cost here.
      // If we input actual cost here, we might not have driver info updated here.
      // So we should just use general "Delivery Expense" or check if `order.deliveryInfo` has existing driver info.

      const driverAffiliation = order.deliveryInfo?.driverAffiliation;
      ...
      // 메모에 배송기사 정보 포함
      const driverInfoStr = driverName && driverContact
        ? `, 배송기사: ${driverName} (${driverContact})`
        : '';
      ...
      const expenseData = {
        ...
        memo: `주문번호: ${order.id}, 고객: ${formData.orderer.name}, 배송지: ${formData.address || '주소 없음'}${driverInfoStr}`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedOrder = {
        ...
        deliveryCostStatus: (formData.receiptType === 'delivery_reservation' && actualDeliveryCost > 0) ? 'completed' : order.deliveryCostStatus,
        deliveryProfit: (formData.receiptType === 'delivery_reservation' && actualDeliveryCost > 0) ? (calculateTotal().deliveryFee - actualDeliveryCost) : order.deliveryProfit
      };

      // 실제 배송비가 입력되었고, 기존 값과 다르거나 새로 입력된 경우 지출 내역 생성
      if (formData.receiptType === 'delivery_reservation' && actualDeliveryCost > 0) {
        // 기존 값과 다르거나, 기존 값이 없었을 때만 생성? 
        // 사용자 요청: "주문현황 페이지에서 주문리스트 수정시 입력란을 만들어줘"
        // 중복 생성을 막기 위해, 값이 변경되었을 때만 생성하는 것이 좋음.
        if (actualDeliveryCost !== order.actualDeliveryCost) {
          await createDeliveryExpense(order, actualDeliveryCost);
        }
      }
