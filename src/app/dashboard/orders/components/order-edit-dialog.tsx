...
export function OrderEditDialog({ isOpen, onOpenChange, order }: OrderEditDialogProps) {
  ...
  const { user } = useAuth();
  const { addExpense, deleteOrderRelatedExpense } = useSimpleExpenses();
  ...
  const createDeliveryExpense = async (order: Order, actualCost: number, actualCostCash: number) => {
    try {
      ...
      if (nonCashAmount > 0) {
        ...
        // 비현금 배송비가 0이거나 없으면 관련 지출 삭제
        await deleteOrderRelatedExpense(order.id, 'DELIVERY');
      }
      ...
      if (actualCostCash > 0) {
        ...
        // 현금 지급액이 0이면 관련 지출 삭제
        await deleteOrderRelatedExpense(order.id, 'DELIVERY_CASH');
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.receiptType === 'delivery_reservation' && deliveryCostFieldsChanged) {
