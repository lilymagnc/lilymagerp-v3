...
const PrintableContent = React.forwardRef<HTMLDivElement, { order: Order; branches: Branch[] }>(({ order, branches }, ref) => {
    const getPrintableData = (order: Order | null, branches: Branch[]) => {
...
export function OrderPrintDialog({ isOpen, onOpenChange, onPrint, order, branches }: OrderPrintDialogProps) {
    if (!order) return null;
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
                ...
                <div className="flex-grow overflow-y-auto border rounded-md">
                   {/* This is a bit of a hack to pass the ref to the printable content */}
                   <div style={{ display: 'none' }}>
                     <PrintableContent ref={(el) => (onPrint as any)._printRef.current = el} order={order} branches={branches} />
                   </div>
