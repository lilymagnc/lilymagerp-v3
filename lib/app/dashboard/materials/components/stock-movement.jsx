...
export function StockMovement() {
    ...
    const handlePhotoChange = (e) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            reader.onloadend = () => {
                setReceiptPhotoPreview(reader.result);
            };
    ...
    const renderList = (list, type) => (<Card>
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {list.length > 0 ? list.map(item => (<TableRow key={item.id}>
    ...
    return (<div>
      <PageHeader title="재고 입출고" description="핸디 스캐너 또는 AI를 사용하여 자재 재고를 관리합니다."/>
      <div className="space-y-6">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <Select onValueChange={setSelectedBranchId} value={selectedBranchId !== null && selectedBranchId !== void 0 ? selectedBranchId : ''}>
                ...
                <SelectContent>
                  {branches.filter(b => b.type !== '본사').map(branch => (<SelectItem key={branch.id} value={branch.id}>{branch.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          </CardContent>
        ...
        <fieldset disabled={!selectedBranchId || materialsLoading} className="space-y-6">
           {materialsLoading && (<div className="flex items-center justify-center gap-2 text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                 <Textarea placeholder="예시) 마르시아 장미 50단, 레드 카네이션 30단, 포장용 크라프트지 10롤..." value={receiptText} onChange={(e) => setReceiptText(e.target.value)} rows={3}/>
                <div>
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleScan} className="flex items-center gap-2">
                ...
                <Input ref={barcodeInputRef} placeholder="바코드 스캔 또는 입력 후 Enter" value={barcode} onChange={(e) => setBarcode(e.target.value)} className="max-w-sm"/>
              </form>
            </CardContent>
          </Card>
