...
export function ProductTable({ products, onSelectionChange, onEdit, onDelete }) {
    ...
    const handlePrintSubmit = ({ quantity, startPosition }) => {
        if (!selectedProduct)
            return;
    ...
    const getStatus = (status, stock) => {
        if (status === 'out_of_stock' || stock === 0)
            return { text: '품절', variant: 'destructive' };
        if (status === 'low_stock' || stock < 10)
            return { text: '재고 부족', variant: 'secondary' };
        return { text: '판매중', variant: 'default' };
    };
    return (<>
      <Card>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-[50px]">
                   <Checkbox checked={isAllSelected} onCheckedChange={(checked) => handleSelectAll(!!checked)} aria-label="모두 선택"/>
                </TableHead>
            ...
            return (<TableRow key={`${product.docId}-${idx}`}>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox checked={!!selectedRows[product.id]} onCheckedChange={() => handleSelectionChange(product.id)} aria-label={`${product.name} 선택`}/>
                  </TableCell>
                  <TableCell className="font-medium cursor-pointer" onClick={() => handleRowClick(product)}>{product.name}</TableCell>
                   <TableCell className="cursor-pointer" onClick={() => handleRowClick(product)}>
                    {product.id && (<Barcode value={product.id} options={{
                        format: 'CODE39',
                        displayValue: true,
                        fontSize: 14,
                        height: 30,
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction className="bg-destructive hover:bg-destructive/90" onClick={() => onDelete(product.docId)}>
                            삭제
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
      ...
      {selectedProduct && (<PrintOptionsDialog isOpen={isPrintDialogOpen} onOpenChange={setIsPrintDialogOpen} onSubmit={handlePrintSubmit} itemName={selectedProduct.name}/>)}
      <ProductDetails isOpen={isDetailOpen} onOpenChange={setIsDetailOpen} product={selectedProduct} onEdit={() => selectedProduct && isHeadOfficeAdmin && handleEdit(selectedProduct)}/>
    </>);
}
