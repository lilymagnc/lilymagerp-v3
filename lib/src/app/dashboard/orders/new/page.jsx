...
export default function NewOrderPage() {
    ...
    const handleContactChange = (e, setter) => {
    ...
    return (<div>
        <PageHeader title="릴리맥 주문테이블" description=""/>
        <Card className="mb-6">
            <CardHeader>
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                  {!selectedBranch ? (<div className="flex items-center gap-2">
                        ...
                        <Select onValueChange={handleBranchChange}>
                            ...
                            <SelectContent>
                                {branches.filter(b => b.type !== '본사').map(branch => (<SelectItem key={branch.id} value={branch.id}>
                                        {branch.name}
                                    </SelectItem>))}
                            </SelectContent>
                        </Select>
            ...
            <div className="md:col-span-2">
              <Card>
                  ...
                  <CardContent className="space-y-6">
                      ...
                      <div>
                          ...
                          <Card className="mt-2">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {orderItems.length > 0 ? (orderItems.map(item => (<TableRow key={item.id}>
                             ...
                             <div className="mt-2 p-2 border-t flex items-center gap-2">
                                {productsLoading ? (<Loader2 className="h-5 w-5 animate-spin"/>) : (<>
                                    <Select onValueChange={(value) => setProductToAdd(branchProducts.find(p => p.id === value) || null)}>
                                        ...
                                        <SelectContent>
                                            {branchProducts.map(p => (<SelectItem key={p.id} value={p.id} disabled={p.stock === 0}>
                                                    {p.name} (재고: {p.stock})
                                                </SelectItem>))}
                                        </SelectContent>
                                    </Select>
                          ...
                          <Card className="mt-2">
                            <CardContent className="p-4 space-y-4">
                                <div>
                                    ...
                                    <RadioGroup value={paymentMethod} onValueChange={(v) => setPaymentMethod(v)} className="flex items-center flex-wrap gap-4 mt-2">
                                 ...
                                 <div>
                                    ...
                                    <RadioGroup value={paymentStatus} onValueChange={(v) => setPaymentStatus(v)} className="flex items-center gap-4 mt-2">
                      ...
                      <div>
                          <Label>주문 유형</Label>
                          <RadioGroup value={orderType} onValueChange={(v) => setOrderType(v)} className="flex items-center gap-4 mt-2">
                      ...
                      <div>
                          <Label>수령 정보</Label>
                          <RadioGroup value={receiptType} onValueChange={(v) => setReceiptType(v)} className="flex items-center gap-4 mt-2">
                          ...
                          {receiptType === 'pickup' && (<Card className="mt-2">
                          ...
                          {receiptType === 'delivery' && (<Card className="mt-2">
                                  <CardContent className="p-4 space-y-4">
                                      <div className="space-y-2">
                                          <Label>배송일시</Label>
                                          <div className="flex gap-2">
                                      ...
                                      <div className="space-y-2">
                                          <Label>배송비</Label>
                                          <RadioGroup value={deliveryFeeType} className="flex items-center gap-4" onValueChange={(value) => setDeliveryFeeType(value)}>
                                          ...
                                          <div className="flex items-center gap-2 mt-2">
                                              ...
                                              {deliveryFeeType === 'manual' && (<Input type="number" placeholder="배송비 직접 입력" value={manualDeliveryFee} onChange={(e) => setManualDeliveryFee(Number(e.target.value))}/>)}
                                          </div>
                                      </div>
                                  </CardContent>
                              </Card>)}
                      ...
                      <div>
                          <Label>메시지</Label>
                          <RadioGroup value={messageType} onValueChange={(v) => setMessageType(v)} className="flex items-center gap-4 mt-2">
