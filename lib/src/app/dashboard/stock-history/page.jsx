...
export default function StockHistoryPage() {
    ...
    return (<div>
      <PageHeader title="재고 변동 기록" description="상품 및 자재의 입출고 내역을 추적하고 관리합니다.">
      ...
      <HistoryFilters filters={filters} onFiltersChange={setFilters} branches={branches}/>
      
      {loading ? (<div className="space-y-2">
            {Array.from({ length: 10 }).map((_, i) => (<Skeleton key={i} className="h-12 w-full"/>))}
        </div>) : (<HistoryTable history={filteredHistory}/>)}
    </div>);
}
