...
export function MaterialTable({ materials, onSelectionChange, onEdit, onDelete }) {
    ...
    const getStatus = (status, stock) => {
        ...
        return { text: '입고중', variant: 'default' };
    };
    return (<>
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            return (<TableRow key={`${material.docId}-${idx}`}>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox checked={!!selectedRows[material.id]} onCheckedChange={() => handleSelectionChange(material.id)} aria-label={`${material.name} 선택`}/>
                  </TableCell>
                  <TableCell className="font-medium cursor-pointer" onClick={() => handleRowClick(material)}>{material.name}</TableCell>
                   <TableCell className="cursor-pointer" onClick={() => handleRowClick(material)}>
                    {material.id && (<Barcode value={material.id} options={{
                        format: 'CODE39',
                        displayValue: true,
                        fontSize: 14,
                        height: 30,
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction className="bg-destructive hover:bg-destructive/90" onClick={() => onDelete(material.docId)}>
                            삭제
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
      ...
      {isStockFormOpen && <StockUpdateForm isOpen={isStockFormOpen} onOpenChange={handleCloseForms} product={selectedMaterial}/>}
       {selectedMaterial && (<PrintOptionsDialog isOpen={isPrintDialogOpen} onOpenChange={setIsPrintDialogOpen} onSubmit={handlePrintSubmit} itemName={selectedMaterial.name}/>)}
      <MaterialDetails isOpen={isDetailOpen} onOpenChange={setIsDetailOpen} material={selectedMaterial} onEdit={() => selectedMaterial && handleEdit(selectedMaterial)}/>
    </>);
}
