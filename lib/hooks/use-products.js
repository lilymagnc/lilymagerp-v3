...
export function useProducts() {
    ...
    const addProduct = async (data) => {
        setLoading(true);
        try {
            const existingProductQuery = query(collection(db, "products"), where("name", "==", data.name), where("branch", "==", data.branch));
    ...
    const updateProduct = async (docId, productId, data) => {
        ...
        try {
            ...
            await setDoc(docRef, Object.assign(Object.assign({}, data), { id: productId }), { merge: true });
        ...
        await Promise.all(dataToProcess.map(async (row) => {
            ...
            try {
                ...
                if (!querySnapshot.empty) {
                    ...
                    await setDoc(newDocRef, Object.assign(Object.assign({}, productData), { id: newId }));
                    newCount++;
                }
            }
            catch (error) {
