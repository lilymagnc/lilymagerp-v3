...
import { collection, getDocs, doc, setDoc, writeBatch, serverTimestamp, runTransaction, query, where, orderBy, limit, deleteDoc, getDoc } from 'firebase/firestore';
...
export function useMaterials() {
            ...
            if (querySnapshot.size <= 1) {
                ...
                if (!initDoc.exists()) {
                    ...
                    const materialsData = seededSnapshot.docs
                        ...
                        .map((doc) => {
                        const data = doc.data();
                        return Object.assign(Object.assign({ docId: doc.id }, data), { status: getStatus(data.stock) });
    ...
    const addMaterial = async (data) => {
        setLoading(true);
        try {
            const existingMaterialQuery = query(collection(db, "materials"), where("name", "==", data.name), where("branch", "==", data.branch));
            ...
            await setDoc(docRef, Object.assign(Object.assign({}, data), { id: newId }));
    ...
    const updateMaterial = async (docId, materialId, data) => {
        ...
        try {
            ...
            await setDoc(docRef, Object.assign(Object.assign({}, data), { id: materialId }), { merge: true });
        ...
        for (const item of items) {
            try {
                await runTransaction(db, async (transaction) => {
                    ...
                    const currentStock = (materialData === null || materialData === void 0 ? void 0 : materialData.stock) || 0;
                    ...
                    historyBatch.set(historyDocRef, {
                        ...
                        branch: branchName,
                        operator: operator,
                        supplier: type === 'in' ? (item.supplier || (materialData === null || materialData === void 0 ? void 0 : materialData.supplier)) : materialData === null || materialData === void 0 ? void 0 : materialData.supplier,
                        price: type === 'in' ? (item.price || (materialData === null || materialData === void 0 ? void 0 : materialData.price)) : materialData === null || materialData === void 0 ? void 0 : materialData.price,
                        totalAmount: type === 'in' ? ((item.price || (materialData === null || materialData === void 0 ? void 0 : materialData.price) || 0) * item.quantity) : 0,
                    });
                });
            }
            catch (error) {
    ...
    const manualUpdateStock = async (itemId, itemName, newStock, branchName, operator) => {
        try {
            await runTransaction(db, async (transaction) => {
                ...
                const currentStock = ((_a = materialDoc.data()) === null || _a === void 0 ? void 0 : _a.stock) || 0;
    ...
    const bulkAddMaterials = async (data, currentBranch) => {
        ...
        await Promise.all(dataToProcess.map(async (row) => {
            var _a, _b;
            try {
                const stock = Number((_b = (_a = row.current_stock) !== null && _a !== void 0 ? _a : row.stock) !== null && _b !== void 0 ? _b : row.quantity);
                if (isNaN(stock))
                    return;
                const materialData = {
                    id: row.id || null,
                ...
                if (!querySnapshot.empty) {
                    ...
                    await setDoc(newDocRef, Object.assign(Object.assign({}, materialData), { id: newId }));
                    newCount++;
                }
            }
            catch (error) {
