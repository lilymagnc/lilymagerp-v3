...
function useBranches() {
    ...
    const fetchBranches = (0, react_1.useCallback)(async () => {
        try {
            setLoading(true);
            const branchesCollection = (0, firestore_1.collection)(firebase_1.db, 'branches');
            const querySnapshot = await (0, firestore_1.getDocs)(branchesCollection);
            // If the collection is empty (or just has the _initialized doc), seed it.
            if (querySnapshot.size <= 1) {
                const initDocRef = (0, firestore_1.doc)(branchesCollection, '_initialized');
                ...
                if (!initDoc.exists()) {
                    ...
                    initialBranches.forEach(branchData => {
                        const docRef = (0, firestore_1.doc)((0, firestore_1.collection)(firebase_1.db, "branches"));
                        batch.set(docRef, branchData);
                    });
                    batch.set(initDocRef, { seeded: true });
                    await batch.commit();
                    // After seeding, refetch to get the fresh data
                    const seededSnapshot = await (0, firestore_1.getDocs)(branchesCollection);
    ...
    const addBranch = async (branch) => {
        try {
            setLoading(true);
            const branchesCollection = (0, firestore_1.collection)(firebase_1.db, 'branches');
            await (0, firestore_1.addDoc)(branchesCollection, branch);
            toast({
                title: '성공',
    ...
    const updateBranch = async (branchId, branch) => {
        try {
            setLoading(true);
            const branchDoc = (0, firestore_1.doc)(firebase_1.db, 'branches', branchId);
            await (0, firestore_1.setDoc)(branchDoc, branch, { merge: true });
            toast({
                title: '성공',
    ...
    const deleteBranch = async (branchId) => {
        try {
            setLoading(true);
            const branchDoc = (0, firestore_1.doc)(firebase_1.db, 'branches', branchId);
            await (0, firestore_1.deleteDoc)(branchDoc);
            toast({
                title: '성공',
